 ...
 */
public class RVCustomBannerView extends FrameLayout {
    ...
    /** 初始化和播放 */
    public RVCustomBannerView(@NonNull Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
    }
    ...
    public void init(List<String> datas, OnItemClickListener listener) {
        mDatas = datas;
        mAdapter = new RVCardBannerAdapter(mDatas, listener);
    ...
    private void initRV() {
        ...
        addView(mRV, new LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT));
    }
    ...
    /* 自定义适配器 */
    private class RVCardBannerAdapter extends RecyclerView.Adapter<RVCardBannerAdapter.ViewHolder> {
        private List<String> mDatas;
        private OnItemClickListener mListener;

        RVCardBannerAdapter(List<String> datas, OnItemClickListener listener) {
            mListener = listener;
            mDatas = datas;
        }

        @NonNull @Override public RVCardBannerAdapter.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
            return new ViewHolder(LayoutInflater.from(parent.getContext()).inflate(R.layout.banner_item, parent, false));
        }

        @Override public void onBindViewHolder(@NonNull RVCardBannerAdapter.ViewHolder holder, int position) {
            int realPosition = position % mDatas.size();

            // 可按需替换使用你项目规定的图片加载框架
            Glide.with(getContext()).load(mDatas.get(realPosition)).into(holder.iv_img);

            if (null != mListener) {
                holder.iv_img.setOnClickListener(v -> mListener.onItemClick(realPosition));
                holder.iv_img.setOnLongClickListener(v -> {
    ...
    private void withTitle() {
        ...
        LayoutParams layoutParams = new LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, dp2px(30));
    ...
    private void withIndicators() {
        ...
        LayoutParams containerParams = new LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, dp2px(30));
